                ...
                if(!prop) { // nicht nur eine Eigenschaft setzen, sondern neues Element konstruieren
                    ...
                    if(noMacro) { // splitted[i] war kein Macro-Aufruf
                        if(splitted[i].search(/^[\[\]].*[\[\]]$/) != -1) { // Gerade, Halbgerade oder Segment
                            ...
                            //console.log("Jessie Script")
                            //console.log(tmp)
                            
                            //Kept running to errors where "pow" or "sin" was not defined; Perhaps something broke in update?
                            //In any case, using mathjs to parse the function allows for **much** greater flexibility and extensibility in generating functions
                            var _f=mathjs.parser().eval(objName+"(x)="+RegExp.$2);
                            var _t=function(x){return _f(x);};
                            //console.log(_f);
